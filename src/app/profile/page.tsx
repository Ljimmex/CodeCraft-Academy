...
export default async function ProfilePage() {
    ...
    try {
        const res = await client.query(
            "SELECT total_xp, created_at, username, bio, badges, certificates, avatar_slug, language FROM users WHERE auth_id = $1",
            [user.id]
        );
        dbUser = res.rows[0];
    } finally {
                    ...
                    <div className="lg:col-span-1 space-y-6">
                        <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 text-center">
                            <div className="relative inline-block mb-4">
                                ...
                                <div className="absolute bottom-0 right-0 w-6 h-6 bg-emerald-500 border-4 border-white dark:border-slate-800 rounded-full" title="Online"></div>
                            </div>

                            <h1 className="text-2xl font-bold text-slate-900 dark:text-white mb-1">{user.displayName || "Anonymous Coder"}</h1>
                            <p className="text-slate-500 dark:text-slate-400 text-sm mb-4">@{dbUser?.username || "username"}</p>
                            ...
                            <a href={stackServerApp.urls.accountSettings} className="btn btn-outline btn-sm w-full dark:text-white dark:border-slate-600 dark:hover:bg-slate-700">
                        ...
                        <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
                            ...
                            {dbUser?.badges && dbUser.badges.length > 0 ? (
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    {/* Map badges here */}
                                    {/* Example Badge */}
                                    <div className="flex flex-col items-center text-center p-4 bg-slate-50 dark:bg-slate-700 rounded-xl border border-slate-100 dark:border-slate-600">
                                        <div className="text-4xl mb-2">🏆</div>
                                        <div className="font-bold text-sm text-slate-900 dark:text-white">First Win</div>
                                        <div className="text-xs text-slate-500 dark:text-slate-400">Solved 1st problem</div>
                                    </div>
                                </div>
                            ) : (
